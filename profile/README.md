...
All packages are available from the openalexPro r-universe:

```r
install.packages(
  c("openalexPro", "openalexSnowball", "openalexConvert", "openalexVectorComp"),
...
# 2. Snowball-expand the seed set (optional)
library(openalexSnowball)
snowball(corpus = "my_corpus", depth = 1, output = "my_corpus_expanded")

# 3. Export to BibTeX for use in a reference manager
...
- **Rust where it matters** — the bulk snapshot converter delegates hot-path work to a compiled Rust back-end (`openalex-core`), with a pure-R/DuckDB fallback for environments without a Rust toolchain.
